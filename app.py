...
class MaxLevelReached(GameError):
    """المهارة وصلت أقصى مستوى ممكن — مفيش معنى نحاول تاني خالص."""
...
# ملاحظة: النص ده تخمين لحد ما نتأكد من رسالة اللعبة الحقيقية لمهارة وصلت
# أقصى مستوى (زي "تقنيات الحرب" اللي بتوقف عند ٤٠٠ في الصور). لو ظهرت
# رسالة رفض متكررة على مهارة معيّنة، ابعتها بالظبط عشان نضيفها هنا بدقة
_MAX_LEVEL_HINTS = [
    "max level", "maximum level", "seviye sınırı", "en yüksek seviye",
    "الحد الأقصى للمستوى",
]
...
def skill_cooldown_seconds(profile, preferred_key=None):
    """
    بيدوّر في بروفايل الحساب على وقت الترقية الشغّالة. لو معدّي preferred_key
    (المهارة اللي البوت بيطوّرها فعلاً دلوقتي)، بيتأكد منها هي الأول قبل ما
    يدوّر على أي مهارة تانية — عشان لو مهارة تانية فيها pending_at قديم أو
    غريب لأي سبب، منجيبش وقت غلط مالوش علاقة بالترقية اللي فعلاً شغّالة.
    بيرجع None لو مفيش ترقية شغّالة خالص.
    """
    ...
    if preferred_key:
        result = _read(preferred_key)
        if result is not None:
            return result

    for key in _ALL_SKILL_KEYS:
        if key == preferred_key:
            continue  # اتفحصت فوق بالفعل
        result = _read(key)
        if result is not None:
            return result
    return None
...
class GameClient:
    ...
    def upgrade_skill(self, skill_key, currency):
        ...
        if any(h in haystack for h in _MAX_LEVEL_HINTS):
            raise MaxLevelReached(message or "المهارة وصلت أقصى مستوى")
...
_scheduler = None  # بيتظبط في bootstrap() — بنستخدمه لجدولة صحوة دقيقة لكل مهمة
...
def queue_task(account_id, kind, payload=None, delay_seconds=0):
    ...
    # النبضة الدورية (كل TICK_SECONDS) شبكة أمان عامة، بس هنا كمان بنجدول
    # صحوة دقيقة لميعاد المهمة دي بالظبط — عشان تتنفّذ لحظة ما تستحق، مش
    # تستنى دورة الفحص العامة الجاية (ممكن تاخد لحد دقيقتين زيادة)
    if _scheduler and delay_seconds > 0:
        try:
            _scheduler.add_job(run_tick, "date", run_date=run_at,
                              args=("precise_wakeup",), misfire_grace_time=120,
                              id=f"wake_{account_id}_{kind}_{run_at.timestamp()}")
        except Exception:
            pass  # لو فشلت الصحوة الدقيقة، النبضة الدورية هتلقط المهمة عادي
...
def task_auto_perk(account, payload):
    ...
    try:
        ...
        # وصلت أقصى مستوى ممكن — مفيش معنى نكرر المحاولة للأبد. نوقف
        # العدّاد هنا، والرسالة الواضحة للمستخدم بتتسجّل من run_task
        db_execute("UPDATE accounts SET perk_next_at=NULL WHERE id=%s", (account["id"],))
        raise
    ...
    try:
        ...
        _apply_profile_snapshot(account["id"], account, profile)
        wait_s = skill_cooldown_seconds(profile, preferred_key=skill_key) or 65
    except Exception:
        wait_s = 65
...
def task_plan_step(account, payload):
    ...
    try:
        ...
        # المهارة وصلت أقصى مستوى ممكن — مفيش معنى نكمل الخطة على المهارة
        # دي. نوقف الخطة كلها، والرسالة الواضحة بتتسجّل من run_task
    ...
    if fresh_profile:
        ...
        wait_s = skill_cooldown_seconds(fresh_profile, preferred_key=skill_key) or 65
    else:
        wait_s = 65
...
def run_task(task):
    ...
    def _clear_stale_ambient_notice(kind_label):
        """
        لو الحساب لسه شايل رسالة خطأ قديمة كانت اتسجّلت للمهمة دي بالظبط
        (من قبل ما نخلّيها هادية)، نمسحها دلوقتي — مش منطقي تفضل معلّقة
        على الشاشة للأبد وهي عمرها ما هتتحدّث تاني لأن المهمة بقت هادية
        """
        if account.get("status") == "error" and f"({kind_label})" in (account.get("last_error") or ""):
            db_execute("UPDATE accounts SET status='ok', last_error='' WHERE id=%s", (account["id"],))
    ...
    try:
        ...
        if task["kind"] not in _AMBIENT_KINDS:
            ...
            _clear_stale_ambient_notice(kind_label)
            _bump_countdown(TICK_SECONDS)

    except MaxLevelReached as e:
        # وصلت أقصى مستوى ممكن — حالة نهائية حقيقية، لازم تبان دايماً حتى
        # لو المهمة من النوع "الهادي" (auto_perk/plan_step)، عشان تعرف إنك
        # محتاج تختار مهارة تانية بدل ما تفضل السلسلة واقفة بصمت من غير سبب
        db_execute("UPDATE tasks SET status='failed', result=%s, attempts=attempts+1 WHERE id=%s",
                   (str(e)[:300], task["id"]))
        kind_label = KIND_LABELS.get(task["kind"], task["kind"])
        mark_error(account, f"{title}: {e} ({kind_label}) — اختار مهارة تانية")
...
def schedule_periodic():
    ...
    for account in accounts:
        ...
        # لو التوكن اتأكد إنه مرفوض فعلاً، منضربش اللعبة بطلبات فاضلة —
        # "تحديث البيانات" بس فوق هو اللي بيفضل يحاول (عشان نكتشف لو
        # التوكن رجع سليم)، باقي الميزات بتستنى لحد ما يتصلّح
        if account.get("token_invalid"):
            continue
    ...
    # تنظيف رسائل خطأ قديمة عالقة من قبل ما ميزات جانبية معيّنة تبقى هادية —
    # مش لازم نستنى دورة الميزة الجاية (ممكن تاخد ساعتين للعسكري مثلاً)
    stale_error_row = db_all("SELECT id, last_error FROM accounts WHERE status='error'")
    for row in stale_error_row:
        text = row.get("last_error") or ""
        if any(f"({KIND_LABELS[k]})" in text for k in _AMBIENT_KINDS if k in KIND_LABELS):
            db_execute("UPDATE accounts SET status='ok', last_error='' WHERE id=%s", (row["id"],))
...
def bootstrap():
    ...
    _scheduler.add_job(run_tick, "interval", seconds=TICK_SECONDS, id="tick",
                      max_instances=1, coalesce=True)
    _scheduler.start()
    log.info("النبضة الداخلية شغّالة كل %s ثانية (زائد صحوة دقيقة لكل مهمة)", TICK_SECONDS)
